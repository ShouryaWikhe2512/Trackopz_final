...
//     return allJobs
//       .filter((job) => job.product.id.toString() === productId)
//       .sort(
//         (a, b) =>
...
//       if (res.ok) {
//         const data = await res.json();
//         const productJob = data.jobs.find(
//           (job: any) =>
...
//           if (moveRes.ok) {
//             setMessage({
//               text: "Product moved to past products successfully!",
//               type: "success",
//             });
...
//                 }
//               });
//               const products = Object.values(latestJobByProduct).map(
//                 (job: any) => ({
//                   id: job.product.id.toString(),
//                   name: job.product.name,
...
//           } else {
//             setMessage({
//               text: "Failed to move product to past",
//               type: "error",
//             });
//           }
//         } else {
...
//             const lastOffJob = sortedJobs
//               .reverse()
//               .find((j) => j.state === "OFF");

//             if (lastOffJob) {
//               // Find the last ON job before this OFF job
//               const jobsBeforeOff = sortedJobs.slice(
//                 sortedJobs.indexOf(lastOffJob) + 1
//               );
//               const lastOnJob = jobsBeforeOff
//                 .reverse()
//                 .find((j) => j.state === "ON");
...
//                   lastOffJob.updatedAt || lastOffJob.createdAt
//                 ).getTime();
...
//                     product.state === "ON" ? "bg-green-500" : "bg-gray-400"
//                   }`}
//                 ></div>
//                 <div>
//                   <h3 className="text-gray-900 font-medium text-base mb-1">
...
//     const totalOn = jobs
//       .filter((j) => j.state === "ON")
//       .reduce((sum, j) => sum + (j.quantity || 1), 0);
//     const totalOff = jobs
//       .filter((j) => j.state === "OFF")
//       .reduce((sum, j) => sum + (j.quantity || 1), 0);
...
export default function WorkPanelInterface() {
  ...
  const getJobsForProduct = (
    ...
    return allJobs
      .filter(
        (job) =>
          job.product.id.toString() === pid &&
          job.machine.name === operation &&
          job.state === state
      )
      .sort(
        (a, b) =>
  ...
  const getLiveProducts = (): Product[] => {
    return productData.filter(
      (product) => product.state === "ON" && product.quantity > 0
    );
  };

  const getPastProducts = (): Product[] => {
    return productData.filter(
      (product) => product.state === "OFF" && product.quantity > 0
    );
  };
    ...
    if (selectedProduct.state === "ON") {
      ...
      if (jobs.length > 0) {
        const start = new Date(
          Math.min(...jobs.map((j) => new Date(j.createdAt).getTime()))
        );
        const end = new Date(
          Math.max(
            ...jobs.map((j) => new Date(j.updatedAt || j.createdAt).getTime())
          )
        );
