...
export default function DispatchPage() {
  ...
  useEffect(() => {
    async function fetchLiveProducts() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const liveProductsArr = data.dispatchableProducts.map(
            (product: any) => ({
              id: product.id,
              name: product.name,
          ...
          if (liveProductsArr.length > 0 && !selectedProduct) {
            ...
            setDispatchForm((prev) => ({
              ...prev,
              product: liveProductsArr[0].name,
            }));
          }
        }
      } catch (error) {
    ...
    const eventSource = new EventSource(
      "/api/live-products/dispatchable/stream"
    );
      ...
      try {
        ...
        if (data.type === "product_moved_to_past") {
          ...
          setLiveProducts((prev) =>
            prev.filter((product) => product.id !== data.data.productId)
          );
          ...
          console.log(
            `Product ${data.data.productName} moved to past products`
          );
        }
      } catch (error) {
  ...
  const handleDispatch = async (): Promise<void> => {
    ...
    const selectedProductData = liveProducts.find(
    ...
    try {
      ...
      if (dispatchRes.ok) {
        ...
        if (moveRes.ok) {
          setMessage({
            text: "Product dispatched successfully and moved to past products!",
            type: "success",
          });
          // Reset form
          setDispatchForm({
          ...
          setMessage({
            text: "Product dispatched but failed to move to past products",
            type: "error",
          });
        }
      } else {
                  ...
                  {isLoading ? (
                    ...
                    <div className="px-4 py-3 text-gray-500 text-center">
                      ...
                      <div className="text-xs text-gray-400 mt-1">
                        Products will appear here when they are in production
                      </div>
                    </div>
                  ) : (
